            ...
            <form method="POST" action="{{ route('contacts.update', $contact) }}" class="bg-surface border border-line rounded-lg p-fluid-md space-y-fluid-sm">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-fluid-sm">
                    <div>
                        ...
                        <x-text-input id="first_name" name="first_name" class="block mt-1 w-full" :value="old('first_name', $contact->first_name)" required autofocus />
                        <x-input-error :messages="$errors->get('first_name')" class="mt-2" />
                    </div>

                    <div>
                        <x-input-label for="last_name" :value="__('Vezetéknév')" />
                        <x-text-input id="last_name" name="last_name" class="block mt-1 w-full" :value="old('last_name', $contact->last_name)" />
