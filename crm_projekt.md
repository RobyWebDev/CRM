...
- **ÚJ (2026-07-25, a `pipeline-sablonok.md` és a `riportok-terv.md` kidolgozása közben felismerve):** a `projects` tábla jelenlegi terve (`adatmodell.md`) egy **egyszeri, határidős megbízást** feltételez (`start_date`/`due_date`/`budget`). A marketing és SEO pipeline-sablonokban viszont megjelenik egy **"Folyamatos kezelés / havi riportolás"** lépés — ami inkább **ismétlődő, havi díjas (retainer) munka**, nem egyszeri projekt. Ez a két modell (egyszeri megbízás vs. havi ismétlődő megbízás) jelenleg nincs külön kezelve az adatmodellben, és ez kihat a leendő számlázásra is (egyszeri számla vs. havi ismétlődő számla). **Nem döntöttem helyette** — ezt Robnak kell tisztáznia, amikor visszatér: (a) elég-e egy egyszerű `is_recurring` + `billing_cycle` mező a `projects` táblán, vagy (b) külön kellene választani "projekt" (egyszeri) és "retainer/előfizetéses megbízás" (ismétlődő) fogalmakat. Nem blokkolja az MVP-t (Robnak elsőre a coaching az elsődleges, ami egyszeri jellegű), de érdemes tisztázni, mielőtt a marketing/SEO ág éles használatba kerül.
